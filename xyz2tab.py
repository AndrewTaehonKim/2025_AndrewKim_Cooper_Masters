#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# modification of xyz2tab by radi0sus @ https://github.com/radi0sus/xyz2tab/blob/main/xyz2tab.py
...
def run_x2t(
    ...
    args_sortasc=False,
    args_sortdes=False,
    args_sortascEl=False,
    args_sortdesEl=False,
    args_includeCon=None,
    args_dihedral=None,
    args_plane1=None,
    args_plane2=None,
    args_radius=8.0,
    ...
    # xx.x% --> 0.xxx
    radii_ext = args_radius / 100

    # element + position in xyz = atom + number, e.g. C --> C0; change to C(0) can be arranged here
    xyz_df["atom1_idx"] = [
        "{}{}".format(atm, idx) for atm, idx in zip(xyz_df.element, xyz_df.index.array)
    ]
    # first atom has now index 1 in atom name, e.g. C --> C1
    if args_index:
        xyz_df["atom1_idx"] = [
            "{}{}".format(atm, idx + 1)
            for atm, idx in zip(xyz_df.element, xyz_df.index.array)
        ]
    # atom 1 is atom 2
    ...
    sum_formula_list = list(
        "{}{}".format(element, number)
        for element, number in zip(info_df.element, info_df.utf_num)
    )
    ...
    # dict
    info_df["atom%"] = info_df.apply(
        lambda x: atomic_weights[x.element] * x.atom1_idx / fw * 100, axis=1
    )
    ...
    info_df["radii_plus"] = info_df.apply(
        lambda x: x.radii + x.radii * radii_ext, axis=1
    )
        ...
        print(
            tabulate(
                [
                    ...
                    [
                        "Excluded atoms    :",
                        re.sub(r"[^a-zA-Z0-9,]", "", str(args_excludeAt)),
                    ],
                    [
                        "Excluded elements :",
                        re.sub(r"[^a-zA-Z0-9,]", "", str(args_excludeEl)),
                    ],
                    [
                        "Included contacts :",
                        re.sub(r"[^a-zA-Z0-9,]", "", str(args_includeCon)),
                    ],
                    ["Covalent radius + :", "{:.2f} %".format(args_radius)],
                ],
                tablefmt="simple",
            )
        )
    ...
    dist_mat_full = pd.DataFrame(
        squareform(pdist(xyz_df.iloc[:, 3:6], "euclid")),
    ...
    # bring it to a "normal" form ...
    dist_df[["atom1_idx", "element1", "cov_radius1"]] = pd.DataFrame(
        dist_df["index"].tolist(), index=dist_df.index
    )
    dist_df[["atom2_idx", "element2", "cov_radius2"]] = pd.DataFrame(
        dist_df["level_1"].tolist(), index=dist_df.index
    )
    ...
    dist_df["distance_radii"] = (dist_df["cov_radius1"] + dist_df["cov_radius2"]) + (
        dist_df["cov_radius1"] + dist_df["cov_radius2"]
    ) * radii_ext
    ...
    if args_includeCon:
        ...
        dist_df.loc[
            (
                dist_df.atom1_idx.isin(args_includeCon)
                & dist_df.atom2_idx.isin(args_includeCon)
                & ~dist_df.is_bond
            ),
            "is_bond",
        ] = True
        # np variant of the above, slightly slower
        # dist_df['is_bond'] = np.where((dist_df.atom1_idx.isin(args_includeCon)) &
        # (dist_df.atom2_idx.isin(args_includeCon) & (~dist_df.is_bond)), True, dist_df['is_bond'])
    ...
    # A0 B1 - --> A0-B1
    dist_df["A-B"] = (
        dist_df["atom1_idx"] + dist_df["fusion_char"] + dist_df["atom2_idx"]
    )

    # A B - --> A-B
    dist_df["El1-El2"] = (
        dist_df["element1"] + dist_df["fusion_char"] + dist_df["element2"]
    )
    ...
    if args_excludeAt:
        sel_dist = all_dist[
            ~all_dist.atom1_idx.isin(args_excludeAt)
            & ~all_dist.atom2_idx.isin(args_excludeAt)
        ]

    # exclude named elements from input (in Element1 and Element2)
    if args_excludeEl:
        sel_dist = all_dist[
            ~all_dist.element1.isin(args_excludeEl)
            & ~all_dist.element2.isin(args_excludeEl)
        ]
    ...
    if args_sortascEl:
        sel_dist = sel_dist.sort_values(
            by=["element1", "element2", "distance_calc", "atom1_idx", "atom2_idx"]
        )
    ...
    grouped = sel_dist[["El1-El2", "distance_calc"]].groupby(
        sel_dist["ElEl"], sort=False
    )

    # verbose table El1-El2 | bond length, e.g. C-C 1.223, 1.456, 1.511
    for groups in grouped:
        summary_bond_table_1.append(
            [
                groups[1].iloc[0].tolist()[0],
                ", ".join(
                    groups[1]
                    .sort_values(by=["distance_calc"])
                    .distance_calc.apply(lambda x: "{:.4f}".format(x))
                    .tolist()
                ),
            ]
        )
    ...
    for groups in grouped:
        summary_bond_table_3.append(
            [
                groups[1].iloc[0].tolist()[0],
                groups[1].distance_calc.count(),
                f"{groups[1].distance_calc.mean():.4f}",
                f"{groups[1].distance_calc.median():.4f}",
                f"{groups[1].distance_calc.std():.4f}",
                f"{groups[1].distance_calc.std(ddof=0):.4f}",
                f"{groups[1].distance_calc.sem():.4f}",
                f"{groups[1].distance_calc.skew():.4f}",
            ]
        )
    ...
    # save summary_bond_table_3 to a data frame
    summary_bond_df = pd.DataFrame(
        summary_bond_table_3,
        columns=[
            "Atoms",
            "Count",
    ...
    dist2_df[["atom1_idx", "element1", "cov_radius1"]] = pd.DataFrame(
        dist2_df["level_0"].tolist(), index=dist2_df.index
    )
    dist2_df[["atom2_idx", "element2", "cov_radius2"]] = pd.DataFrame(
        dist2_df["level_1"].tolist(), index=dist2_df.index
    )
    ...
    # column with the sum of the atomic radii from two elements /atoms + x%
    dist2_df["distance_radii"] = (dist2_df["cov_radius1"] + dist2_df["cov_radius2"]) + (
        dist2_df["cov_radius1"] + dist2_df["cov_radius2"]
    ) * radii_ext
    # distance is considered as bond if the calculated distance is smaller than the sum of the atomic radii
    # set to 'True' if this is a bond
    dist2_df["is_bond"] = (dist2_df["distance_calc"] > 0) & (
        dist2_df["distance_calc"] < dist2_df["distance_radii"]
    )
    ...
    if args_includeCon:
        ...
        dist2_df.loc[
            (
                (dist2_df.distance_calc > 0)
                & dist2_df.atom1_idx.isin(args_includeCon)
                & dist2_df.atom2_idx.isin(args_includeCon)
                & ~dist2_df.is_bond
            ),
            "is_bond",
        ] = True
        ...
        (
            {
                ...
                "atom2_idx": atom2,
                "atom3_idx": atom3,
                "angle_calc": anglelist,
            }
        )
    )
    # construct elements from atom names, e.g. C1 --> C, Fe13 --> Fe
    angles_df["element1"] = angles_df["atom1_idx"].apply(
        lambda x: re.sub(r"\d+", "", x)
    )
    angles_df["element2"] = angles_df["atom2_idx"].apply(
        lambda x: re.sub(r"\d+", "", x)
    )
    angles_df["element3"] = angles_df["atom3_idx"].apply(
        lambda x: re.sub(r"\d+", "", x)
    )
    ...
    angles_df["El1-El3"] = (
        angles_df["element1"] + angles_df["fusion_char"] + angles_df["element3"]
    )
    ...
    if args_excludeAt:
        sel_angles = angles_df[
            ~angles_df.atom1_idx.isin(args_excludeAt)
            & ~angles_df.atom2_idx.isin(args_excludeAt)
            & ~angles_df.atom3_idx.isin(args_excludeAt)
        ]

    # exclude named elements from input (in Element1 and Element2 and Element3)
    if args_excludeEl:
        sel_angles = angles_df[
            ~angles_df.element1.isin(args_excludeEl)
            & ~angles_df.element2.isin(args_excludeEl)
            & ~angles_df.element3.isin(args_excludeEl)
        ]
    ...
    grouped = sel_angles[["El1-El2-El3", "angle_calc"]].groupby(
        sel_angles["ElEl_ElElEl"], sort=False
    )

    # verbose table El1-El2-El3 | angle, e.g. C-C-C 122.31, 145.61, 151.11
    for groups in grouped:
        summary_angle_table_1.append(
            [
                groups[1].iloc[0].tolist()[0],
                ", ".join(
                    groups[1]
                    .sort_values(by=["angle_calc"])
                    .angle_calc.apply(lambda x: "{:.2f}".format(x))
                    .tolist()
                ),
            ]
        )
    ...
    for groups in grouped:
        summary_angle_table_3.append(
            [
                groups[1].iloc[0].tolist()[0],
                groups[1].angle_calc.count(),
                f"{groups[1].angle_calc.mean():.2f}",
                f"{groups[1].angle_calc.median():.2f}",
                f"{groups[1].angle_calc.std():.2f}",
                f"{groups[1].angle_calc.std(ddof=0):.2f}",
                f"{groups[1].angle_calc.sem():.2f}",
                f"{groups[1].angle_calc.skew():.2f}",
            ]
        )
    ...
    if args_dihedral:
        a1 = xyz_df.index[xyz_df["atom1_idx"] == args_dihedral[0]].tolist()
        a2 = xyz_df.index[xyz_df["atom1_idx"] == args_dihedral[1]].tolist()
        a3 = xyz_df.index[xyz_df["atom1_idx"] == args_dihedral[2]].tolist()
        a4 = xyz_df.index[xyz_df["atom1_idx"] == args_dihedral[3]].tolist()
        try:
    ...
    if args_plane1:
        ...
        a1 = xyz_df.index[xyz_df["atom1_idx"].isin(args_plane1)].tolist()
        # check for range notation symbol ':'
        if ":" in args_plane1:
            # get indices for ':'
            sep_indices = [i for i, x in enumerate(args_plane1) if x == ":"]
            ...
            for sep_idx in sep_indices:
                ...
                if sep_idx == 0:
                    ...
                    try:
                        ...
                        start = xyz_df.index[
                            xyz_df["atom1_idx"] == args_plane1[sep_idx - 1]
                        ].values.astype(int)[0]
                    except IndexError:
        ...
        # if not, warning
        atom_names_in_arg = [x for x in args_plane1 if x != ":"]
    ...
    if args_plane2:
        ...
        # get the index of the selected atoms
        a2 = xyz_df.index[xyz_df["atom1_idx"].isin(args_plane2)].tolist()

        if ":" in args_plane2:

            sep_indices2 = [i for i, x in enumerate(args_plane2) if x == ":"]
            ...
            for sep_idx2 in sep_indices2:
                ...
                if sep_idx2 == 0:
                    ...
                    try:
                        start2 = xyz_df.index[
                            xyz_df["atom1_idx"] == args_plane2[sep_idx2 - 1]
                        ].values.astype(int)[0]
                    except IndexError:
                        print("")
                        print("Warning! Malformed input.")
        ...
        atom_names_in_arg2 = [x for x in args_plane2 if x != ":"]
    ...
    if args_show or args_showbl or args_shownl:
        ...
        bond_label = (
            sel_dist2["distance_calc"].apply(lambda x: "{:.3f}".format(x)).tolist()
        )
        ...
        for bonds, labels in zip(atom1_2_coord, bond_label):
            ...
            if args_showbl:
                # show bond labels
                ax.text(
                    *np.average(bonds + 0.06, axis=0).T,
                    labels,
                    fontsize=(100 / len(xyzarr) + 8) / 1.5,
                    color="gray",
                )
        ...
        if args_plane1:
            ...
            if args_plane2:
                ...
                d2 = -c2.dot(n2)
            # plane 1 equation
            z1 = (-n1[0] * xx1 - n1[1] * yy1 - d1) * 1.0 / n1[2]
            if args_plane2:
                # plane 2 equation
                z2 = (-n2[0] * xx2 - n2[1] * yy2 - d2) * 1.0 / n2[2]
            # plot plane 1
            surf = ax.plot_surface(
                xx1, yy1, z1, color="blue", alpha=0.3, label="Plane 1"
            )

            if args_plane2:
                # plot plane 2
                surf = ax.plot_surface(
                    xx2, yy2, z2, color="red", alpha=0.3, label="Plane 2"
                )

        # show arrows representing the xyz-axes, starting from 0,0,0
        if args_showori:
            arrow_length = (
                sum(abs(i) for i in ax.get_xlim())
                + sum(abs(i) for i in ax.get_ylim())
                + sum(abs(i) for i in ax.get_zlim())
            )
            arrow_length = (arrow_length / 3) * 0.5
            if arrow_length > 3:
                arrow_length = 3
            ax.arrow3D(
                0, 0, 0, 0, 0, arrow_length, mutation_scale=20, ec="black", fc="red"
            )
            ax.arrow3D(
                0, 0, 0, 0, arrow_length, 0, mutation_scale=20, ec="black", fc="green"
            )
            ax.arrow3D(
                0, 0, 0, arrow_length, 0, 0, mutation_scale=20, ec="black", fc="blue"
            )
    ...
    return summary_bond_df, summary_angle_df